  ...
  const SidebarContent = () => (
    <>
      <div className="p-4 border-b border-border h-20 flex items-center justify-between">
        {!isSidebarCollapsed && <h1 className="text-2xl font-bold text-foreground">Admin Panel</h1>}
        <button
      ...
      <nav className="px-2 py-4 overflow-y-auto flex-1">
        <ul className="space-y-1">
          {navItems.map((item) => {
            const isActive = item.href === '/admin' ? router.pathname === item.href : router.pathname.startsWith(item.href);
